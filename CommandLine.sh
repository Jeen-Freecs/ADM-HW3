# merge 
cat course_*.tsv | sed 's/$/\n/' >> merged_courses.tsv

cat course_1.tsv > merged_courses.tsv
for i in {2..6000}; do cat course_$i.tsv >> merged_courses.tsv; done

find . -name "course_*.tsv" -print0 | xargs -0 cat > merged_courses.tsv
